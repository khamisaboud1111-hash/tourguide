...
import {
  ...
  Star, BookOpen, Settings, Image, Bell, CalendarRange, PlusCircle, FolderTree, DollarSign,
...
type NavGroup = { titleKey: string; items: NavItem[]; collapsible?: boolean };
...
export default function AdminDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-stone-100">
      <BookingNotifier />
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 max-w-[85vw] bg-white shadow-xl transform transition-transform duration-300 ease-out ${drawerOpen ? "translate-x-0" : "-translate-x-full"}`}
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-4" aria-label="Admin navigation">
          {navGroups.map((group) => {
            const groupIsActive = isActive(group.items[0]?.href);
            return (
              <div key={group.titleKey} className="mb-4">
                <p className="text-[11px] uppercase tracking-widest text-stone-400 font-semibold mb-2">
                  {t(group.titleKey)}
                </p>
                ...
                {group.collapsible && (
                  <div className="mt-2 pt-2 border-t border-stone-200">
                    <Link
                      ...
                      className="flex items-center gap-2 px-3 py-1.5 text-sm text-stone-500 hover:text-indigo-600 transition-colors"
                      title="Add new">
                      <PlusCircle size={14} /> {t("adminAddNew")}
                    </Link>
                  </div>
                )}
              </div>
