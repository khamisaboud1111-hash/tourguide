  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl font-semibold">Promotions &amp; Coupons</h1>
        <Button icon={<Plus size={16} />}>New promotion</Button>
      </div>
      ...
      <div className="rounded-2xl border border-stone-200 bg-white overflow-hidden">
        <table className="w-full">
          <thead className="bg-stone-50 border-b border-stone-200">
            <tr>
              {["Code", "Type", "Value", "Min spend", "Usage", "Expiry", "Status", ""].map((h) => (
          ...
          <tbody className="divide-y divide-stone-100">
            {promotions.map((p) => (
              <tr key={p.id} className="hover:bg-stone-50">
                ...
                <td className="px-4 py-3">
                  <Badge variant={p.active ? "success" : "default"}>{p.active ? "Active" : "Paused"}</Badge>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => toggle(p.id)} className="text-stone-400 hover:text-clove-600 transition-colors" aria-label={p.active ? "Pause promotion" : "Activate promotion"}>
                      {p.active ? <ToggleRight size={20} /> : <ToggleLeft size={20} />}
                    </button>
                    <button onClick={() => remove(p.id)} className="text-stone-400 hover:text-clove-600 transition-colors" aria-label={`Delete ${p.code}`}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
        ...
        {promotions.length === 0 && (
          <p className="text-sm text-stone-500 p-6 text-center">No promotions yet. Create one to drive bookings.</p>
        )}
      </div>
    </div>
  );
