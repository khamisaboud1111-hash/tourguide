        ...
        <div className="md:col-span-2 grid grid-cols-4 md:grid-cols-2 gap-3 md:gap-4">
          {thumbs.slice(0, 4).map((p, i) => (
            <button
              ...
              aria-label={t("viewImage").replace("{n}", String(i + 1))}
            >
              ...
              {i === 3 && thumbs.length > 4 && (
                <span className="absolute inset-0 bg-indigo-900/60 flex items-center justify-center text-white text-sm font-medium">
                  {t("moreCount").replace("{n}", String(thumbs.length - 4))}
                </span>
              )}
            </button>
          ))}
      ...
      {open && (
        ...
        >
          <div className="flex items-center justify-between px-4 md:px-6 py-4 text-stone-100">
            ...
            <button onClick={() => setOpen(false)} aria-label={t("close")} className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white">
              <X size={18} />
            </button>
          </div>
          <div className="flex-1 relative flex items-center justify-center p-4 md:p-8">
            ...
            {photos.length > 1 && (
              <>
                <button onClick={() => setActive((a) => (a === 0 ? photos.length - 1 : a - 1))} aria-label={t("previous")} className="absolute left-4 md:left-8 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/15 hover:bg-white/25 text-white backdrop-blur">
                  <ChevronLeft size={20} />
                </button>
                <button onClick={() => setActive((a) => (a === photos.length - 1 ? 0 : a + 1))} aria-label={t("next")} className="absolute right-4 md:right-8 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/15 hover:bg-white/25 text-white backdrop-blur">
