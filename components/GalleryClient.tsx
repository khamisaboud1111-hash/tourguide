    ...
    <>
      <div className="columns-2 md:columns-3 gap-4 space-y-4 max-h-[85vh] overflow-y-auto pr-1 overscroll-contain">
        {allPhotos.map((p, idx) => (
          ...
          >
            <Image
              src={resolveSrc(p.seed, 1200, p.tall ? 1600 : 1200)}
              alt={p.alt}
              fill
      ...
      {open !== null && current && (
        <motion.div
          ...
          <div className="flex-1 relative flex items-center justify-center p-4">
            <Image src={resolveSrc(current.seed, 3840, 2160)} alt={current.alt} fill className="object-contain p-4 md:p-10" sizes="100vw" quality={100} unoptimized={current.seed.startsWith("http")} />
