...
export default function Footer() {
  const lang = (() => {
    try { return getLang(); } catch { return "en" as const; }
  })();
    ...
    <footer className="bg-indigo-800 text-stone-100">
      ...
      <div className="container-page py-10 md:py-14">
        <div className="rounded-[20px] bg-indigo-700/70 border border-indigo-600/60 p-6 md:p-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6 backdrop-blur">
          <div>
            <p className="text-saffron-300 text-xs uppercase tracking-[0.18em] font-medium mb-2">{t("footerReady")}</p>
            <h3 className="font-display text-2xl md:text-3xl font-semibold text-white text-balance">{t("footerPlan")}</h3>
      ...
      <div className="container-page py-10 grid gap-10 md:grid-cols-12 border-t border-indigo-700/60">
        ...
        <div className="md:col-span-2">
          ...
          <ul className="space-y-2.5 text-sm text-stone-300">
            <li><Link href="/about" className="hover:text-white transition-colors">{t("aboutGuide")}</Link></li>
            <li><Link href="/contact" className="hover:text-white transition-colors">{t("contact")} &amp; {t("meetingPoint")}s</Link></li>
            <li><Link href="/journal" className="hover:text-white transition-colors">{t("journal")}</Link></li>
            <li><Link href="/gallery" className="hover:text-white transition-colors">Zanzibar {t("gallery")}</Link></li>
          </ul>
        </div>
